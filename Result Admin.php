...
$query1 = "SELECT votenumbers.countno, candidates.const, candidates.states, candidates.partysymbol,COUNT(countno) AS 'Vote Count', MAX(countno) AS 'Max values' FROM votenumbers INNER JOIN candidates ON votenumbers.countno = candidates.ID WHERE ID=countno GROUP BY const, party ORDER BY COUNT(countno) ASC";
